...
const messageMap: Array<[RegExp, string]> = [
  ...
  [/is_active\s*(=|:)\s*false|account is not active|inactive|not active/i, "Ваш аккаунт создан. Подтвердите аккаунт в письме, отправленному по указанному почтовому адресу."],

  [/value is not a valid email/i, "Введите корректный email."],
  [/\bemail\b\s*(is|should be|invalid|not valid)\b/i, "Введите корректный email."],
...
export const getFriendlyError = (
  payload: unknown,
  fallback = "Что-то пошло не так. Проверьте данные и попробуйте еще раз.",
) => {
  const raw =
